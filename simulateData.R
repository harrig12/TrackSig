...
# This script creates simulated data of several types. It has been prepared for NCOMMS-17-31183B

# The following experiments are simulated 
# 1. Tumours sequenced at varying depths
# 2. Tumours sequenced at varying depths, for assessment at different bin sizes
# 3. Tumours that have neutrally evolving mutations

# =======================================================================
# Load trinucleotide annotation
# =======================================================================

trinucleotide_internal <- read.table("annotation/trinucleotide.txt")
...
write_sim_annotation <- function(simulation_name, sig_activities, sig_header,
  ...
  # Save active signatures generated by generate_ccf_simulation() to a file
...
generate_ccf_simulation <- function(
  ...
  add_neutral_mutations = FALSE, neutralFromClonalOnly = TRUE)
{
  ...
  for (cl in 1:n_clusters) {
    ...
    add_neutral_mutations_thisCl <- add_neutral_mutations

    # don't add from all clusters if want only from clonal
     if(add_neutral_mutations & neutralFromClonalOnly){
  	if(cl != 1){
	    add_neutral_mutations_thisCl <- FALSE
	}	
    }	
 
    
    if (add_neutral_mutations_thisCl) {
      eff_mut_rate = 20
      min_mut_reads = 3
      vaf_cutoff = min_mut_reads / mean_depth
      cluster_vaf = cluster_ccf / 2
       
      # As per Williams et al., 2016. "Identification of neutral tumor evolution across cancer types."
      n_neutral_mut = as.integer(eff_mut_rate * (1/vaf_cutoff - 1/cluster_vaf))
       
      if (n_neutral_mut > 0) {
         
        print(paste0("number of neural mutations in cluster ", cl))
        print(n_neutral_mut)
        print("depth")
        print(mean_depth)
        ...
        # Sample depth from Poisson (lambda = mean depth)
        depths =  c(depths, rpois(n_neutral_mut, lambda = mean_depth))
         
        # No CNA changes
        mut_alleles = c(mut_alleles, rep(normal_mut_alleles, n_neutral_mut))
        total_CN = c(total_CN, rep(normal_total_CN, n_neutral_mut))
         
        
        uniform_samples = runif(n_neutral_mut, min = 0., max = 1.)
         
        # Generate vafs for neutral mutations by sampling from power-law distribution on interval [vaf_cutoff, cluster_vaf]
        # Sampling from power-law is performed by taking uniform samples and using invert CDF to convert them into samples from power-law
        get_power_law_samples <- function(unif_samples, f_min, f_max){
          # use inverse CDF of power law distribution
          return(1 / (1/f_max - unif_samples * (1/f_max - 1/f_min)))
        }
         
        power_law_samples = get_power_law_samples(uniform_samples, vaf_cutoff, cluster_vaf)
         
        prob = c(prob, power_law_samples)
         
        n_mut <- n_mut + n_neutral_mut
      }
    }
  ...
  filter_vi <- function(df){
    #remove mutations with v_i <= 2
    return(df[(df$n_alt_alleles >2),])
  }
  
  data_all_clusters <- lapply(data_all_clusters, filter_vi)
  
  data_all_clusters_list <- data_all_clusters
  data_all_clusters_table <- do.call(rbind, data_all_clusters)
   
  print("Total number of mutations per simulation")
  print(dim(data_all_clusters_table))
...
create_simulation_set_depths <- function(outdir = "simulations", mut_per_sim = 5000,
...
create_simulation_bin_sizes <- function(outdir = "simulations", mut_per_sim = 5000,
                                  sim_activity_file = "annotation_bin_simulations/sim_active_in_sample.txt",
...
create_simulation_set_neutralEvol <- function(outdir = "simulations", mut_per_sim = 5000,
                                  sim_activity_file = "annotation/sim_active_in_sample.txt",
                                  sim_purity_file = "annotation/sim_purity.txt",
                                  sim_tumortype_file = "annotation/sim_tumortypes.txt",
                                  signature_file = "annotation/sigProfiler_SBS_signatures.txt",
  ...
  print("Simulation type 4a: one cluster with neutral mutations")
  # signatures change in one cluster but not in the other"
  n_simulations = 100
  ...
  print("Simulation type 4b: Neutral evolution with two clusters")
  # Add the neutral evolution "tail" according to 1/f^2, where f is the mutation frequency
  # Neutral evoltion paper: "Quantification of subclonal selection in cancer from bulk sequencing data" Williams et al, 2018
  n_simulations = 100
...
create_simulation_set_depths(outdir = "data_depths")
create_simulation_bin_sizes(outdir = "data_bin_sizes", mut_per_sim = 6000)
create_simulation_set_neutralEvol(outdir = "data_neutralEvol")
